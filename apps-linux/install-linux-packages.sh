...
install_ov() {
  ...
  if command -v ov &>/dev/null; then
    info "ov already installed, skipping..."
    return
  fi
  info "Installing ov..."
  local tmp
  tmp=$(mktemp -d)
  curl -fsSL "https://github.com/noborus/ov/releases/download/v${version}/ov_${version}_${ARCH}.deb" -o "$tmp/ov.deb"
  sudo dpkg -i "$tmp/ov.deb"
  rm -rf "$tmp"
}
...
install_lefthook() {
  ...
  if command -v lefthook &>/dev/null; then
    info "lefthook already installed, skipping..."
    return
  fi
  info "Installing lefthook..."
  local tmp
  tmp=$(mktemp -d)
  curl -fsSL "https://github.com/evilmartians/lefthook/releases/download/v${version}/lefthook_${version}_${ARCH}.deb" -o "$tmp/lefthook.deb"
...
install_yamlfmt() {
  ...
  install_gh_tarball "yamlfmt" \
    "https://github.com/google/yamlfmt/releases/download/v${version}/yamlfmt_${version}_Linux_${arch_name}.tar.gz" \
    "yamlfmt"
}
...
install_editorconfig_checker() {
  ...
  if command -v editorconfig-checker &>/dev/null; then
    info "editorconfig-checker already installed, skipping..."
    return
  fi
  info "Installing editorconfig-checker..."
  local tmp
  tmp=$(mktemp -d)
  curl -fsSL "https://github.com/editorconfig-checker/editorconfig-checker/releases/download/v${version}/editorconfig-checker_${version}_linux_${ARCH}.deb" -o "$tmp/ec.deb"
  sudo dpkg -i "$tmp/ec.deb"
  rm -rf "$tmp"
}
...
install_fx() {
  local version="39.2.0"
  install_gh_binary "fx" \
    "https://github.com/antonmedv/fx/releases/download/${version}/fx_linux_${ARCH}"
}
...
info "Setting zsh as defaul shell"
chsh -s $(which zsh)

info "Linux packages installed."
